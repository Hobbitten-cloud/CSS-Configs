...
function CalculateDynamicQuota()
{
    ...
    {
        if (player.IsValid() && player.GetTeam() == 3 && player.GetHealth() > 0)
        {
            humanCount++;
        }
    }
...
function PollDeadPlayers()
{
    ...
    while ((player = Entities.FindByClassname(player, "player")) != null && (!player.IsValid() && player.GetTeam() != 3 && player.GetTeam() != 0))
    {
    ...
    EntFire("quota_script", "RunScriptCode", "quota_script.PollDeadPlayers();", 10);
}
